...
function DynamicTwoInput({ data, addElement, updateElement, deleteElement, saveElement, uploadImages }) {
  ...
  const [imagePreviews, setImagePreviews] =useState(Array(data.length).fill([]));

  const [imageFiles, setImageFiles] = useState(Array(data.length).fill(null));
  ...
  const addImage = (e, formIndex) => {
    ...
    updatedData[formIndex].file = fileArray;
  
    updateElement(formIndex, 'file', fileArray);
  
    setSelectedImages(prevImages => [...prevImages.slice(0, formIndex), fileArray, ...prevImages.slice(formIndex + 1)]);
    ...
    Promise.all(filePreviews)
      .then(previews => {
        setImagePreviews(prevPreviews => [...prevPreviews.slice(0, formIndex), previews, ...prevPreviews.slice(formIndex + 1)]);
      })
  ...
  const handleClick = () => {
    ...
    if (hasEmptyFields) {
      ...
      const newData = data.map(val => ({ ...val }));
  ...
  {data.map((val, i) => (
    <Col lg={4} key={i}>
      <div className='main'>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap' }}>
  {imagePreviews[i] && imagePreviews[i].map((preview, index) => (
    <div key={index} style={{ position: 'relative' }}>
      <img src={preview} style={{ width: '100px', height: '100px', marginRight: '10px', marginBottom: '10px' }} />
      <MdDelete style={{ position: 'absolute', padding: 0, marginRight: 87, color: 'white', background: 'red', fontSize: 23, top: 0, right: 0, cursor: 'pointer' }} onClick={() => handleImageDelete(i, index)} />
    </div>
  ))}
</div>
